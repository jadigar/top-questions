export function isValidSudokuOnePass(board: string[][]): boolean {
  const rowMap = new Array(9).fill(0).map((v) => new Set<string>());
  const columnMap = new Array(9).fill(0).map((v) => new Set<string>());
  const boxMap = new Array(9).fill(0).map((v) => new Set<string>());
