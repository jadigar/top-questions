...
export function shortestBridge(grid: number[][]): number {
  const n = grid.length;
  const queue: Coordinate[] = []; // Queue to hold coordinates
  let found = false;
  ...
  while (queue.length > 0) {
    ...
    for (let i = 0; i < size; i++) {
      ...
      const [x, y] = queue.shift()!; // Remove and retrieve the first element of the queue
      for (const [dx, dy] of directions) {
        // Go through each possible direction
        const nx = x + dx;
        const ny = y + dy; // Determine the new point
